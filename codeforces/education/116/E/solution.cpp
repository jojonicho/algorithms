...
ll dp[N][X]; // i heroes alive, health > j

int n, x;

ll go(int curN, int curX) {
  if(curN == 0)
    return 1;
  if(curN == 1 || curX == x)
    return 0;
  ...
  res = 0;
  int nxtX = min(x, curX + curN - 1);
  for(int k=curN; k>=0; k--) {
    // how many ways are there to make (curN - k) heroes die
    int nDie = curN - k; // nDie people have health, curX < health <= nxtX
    ll c = mul(bin_pow(nxtX - curX, nDie), nCk(curN, nDie));
    ll cur = mul(c, go(k, nxtX));
    res = add(res, cur);
  }
  //dbg() << im(curN) im(curX) im(res);
  return res;
}
