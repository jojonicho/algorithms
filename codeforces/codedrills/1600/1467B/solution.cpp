...
void solve() {
  ...
  for(int i=1; i<=n; i++) {
    pref[i] = pref[i-1] + isBob(i);
  }

  // find up down or down up
  pref[n] = pref[n-1];
  int ans = pref[n];
  for(int i=2; i<n; i++) {
    int x = A[i];

    int oriCost = pref[i+1] - pref[i-2];
    auto getCost=[&](int i) {
      int curCost = isBob(i) + isBob(i-1) + isBob(i+1);
      return oriCost - curCost;
    };
    // change to A[i-1]
    A[i] = A[i-1];
    chmin(ans, pref[n] - getCost(i));
