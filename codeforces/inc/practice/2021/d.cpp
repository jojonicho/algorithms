...
struct SegTree {
  ...
  void app(int i, T x, int l, int r) {
    tree[i].sm = (tree[i].sm + (r - l + 1) * x) % M;
    tree[i].lz = (tree[i].lz + x) % M;
  }
  ...
  void upd2(int a, int b, T x, int i = 1, int l = 1, int r = n) {
    ...
    tree[i].sm = (tree[2 * i].sm + tree[2 * i + 1].sm) % M;
  }
  ...
  T qry(int a, int b, int i = 1, int l = 1, int r = n) {
    ...
    int m = (l + r) / 2;
    psh(i, l, m, r);
    return (qry(a, b, i * 2, l, m) + qry(a, b, i * 2 + 1, m + 1, r)) % M;
  }
};
