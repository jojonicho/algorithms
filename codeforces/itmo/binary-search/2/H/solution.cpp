...
void solve() {
  ...
  // count needed for a sandwich
  int cb = count(all(s), 'B');
  int cs = count(all(s), 'S');
  int cc = count(all(s), 'C');

  ll lo = 0, hi = 1e12 + 100;
  while(lo <= hi) {
    ll mid = lo + (hi-lo)/2;
    // number of ingredients needed
    ll b = max(0LL, mid * cb - nb);
    ll s = max(0LL, mid * cs - ns);
    ll c = max(0LL, mid * cc - nc);

    ll cur = b * pb + s * ps + c * pc;
    if(cur <= r)
      lo = mid+1;
    else
      hi = mid-1;
