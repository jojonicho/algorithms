...
int main() {
  ...
  while(!A[0].empty() && !A[1].empty() && !A[2].empty() && !A[3].empty()) {
    vt<int> cur = {{A[0].back(), A[1].back(), A[2].back(), A[3].back()}};
    int mx = *max_element(all(cur)), mn = *min_element(all(cur));
    if(mx-mn < dist) {
      dist = mx-mn;
      ans = cur;
    }
