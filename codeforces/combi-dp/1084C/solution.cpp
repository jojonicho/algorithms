...
int main() {
  ...
  // number of segment lengths = 2 ^ m - 1

  int ans = 1;
  // multiply with size of each component

  for(auto c: cnts)
    ans = mul(ans, c + 1); // multiple each valid endpoint, + 1 to include an empty endpoint

  // remove the empty set
  cout << add(ans, MOD-1) << en;
}
