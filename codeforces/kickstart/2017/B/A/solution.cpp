...
void solve() {
  ...
  for(int i=0; i<n; i++) {
    ll nBiggest = 0;
    if(i-1 >= 0) nBiggest = bin_pow(2, i) - 1;
    ll nSmallest = 0;
    if(i+1 < n) nSmallest = bin_pow(2, n-(i+1)) - 1;
    //ll cur = left + right;
    //dbg() << im(i) im(A[i]) im(nBiggest) im(nSmallest);
    //ans += nBiggest * A[i] - nSmallest * A[i];
    ans = add(ans, mul(nBiggest, A[i]));
    ans = sub(ans, mul(nSmallest, A[i]));
  }
  cout << ans << en;
}
