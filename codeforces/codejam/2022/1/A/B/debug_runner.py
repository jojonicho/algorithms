...
class ProxySubprocessThread(threading.Thread):
  def __init__(self,
               args,
               stdin_pipe=subprocess.PIPE,
               stdout_pipe=subprocess.PIPE,
  ...
  def report(self, message, party):
    ...
    if party == 'USER':
      ...
      a = "JUDGE"
      b = ""
    print("{:>6} {:>6} {:<6} {}".format(self.ix, a, b, message))
    self.ix += 1
  ...
  def run(self):
    while True:
      ...
      if not data:break
      self.report(data.decode(), "JUDGE")
      self.stdout_pipe.write(data)
      self.stdout_pipe.flush()

      data = self.stdin_pipe.readline()
      if not data: break
      self.report(data.decode(), "USER")
...
if '-debug' in sys.argv:
  ...
  SThread = ProxySubprocessThread
else:
  SThread = SubprocessThread
...
t_judge = SThread(judge_args, stdin_pipe=t_sol.p.stdout,
