...
https://i.loli.net/2021/06/13/wsnLybBfjASQkuX.png
*/
...
int main() {
  ...
  FOR(h) G.add_edge(s, i, 1);
  FOR(w) G.add_edge(h + i, t, 1);

  FOR(n) {
    int a, b, c, d;
    cin >> a >> b >> c >> d;
    G.add_edge(h + w + i, h + w + n + i, 1);
    for (int j = a - 1; j < c; j++) G.add_edge(j, h + w + i, 1);
    for (int j = b - 1; j < d; j++) G.add_edge(h + w + n + i, h + j, 1);
  }
  cout << G.flow(s, t) << en;
}