...
void solve() {
  // if building decreases, it has to be 0
  int n, k;
  cin >> n >> k;

  int ans = 0;
  for(int i=0; i<=min(n, k); i++) { // number of empty buildings
    int stars = i;
    int m = n-i;
    int cur = nCk(n, i); // choose the 0 spots
    cur = mul(cur, nCk(i+m-1, m-1)); // stars and bars
    ans = add(ans, cur);
  }
