...
/**
 * or use the formula nCr = n-1Cr + n-1Cr-1
 */
const int N = 60;
ll dp[N + 1][N + 1];
...
int main() {
  ...
  string s;
  while (a + b) {
    /* try to put "a", if a+b-1 C a-1 -> the number of ways if we choose to put
     * "a" here if the result is less then k, then the right segment ->
     * dp[a+b-1][a-1] or dp[a+b-1][b] <= k wont have sufficient number of
     * arrangements. We put "b" instead
     */
    ll max_k_put_a = dp[a + b - 1][a - 1];
    if (max_k_put_a <= k) {
      k -= max_k_put_a;
      s += "b";
      b--;
