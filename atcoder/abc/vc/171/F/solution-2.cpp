...
void solve() {
  ...
  // to avoid overcounting, only allow the letter s[i] for either left or right
  // just like a grammar

  int ans = 0;
  for(int i=0; i<=k; i++) { // the number of elements on the left side
    int good = binpow(25, i);
    good = mul(good, binpow(26, k-i));
    good = mul(good, nCk(i+n-1, n-1));
