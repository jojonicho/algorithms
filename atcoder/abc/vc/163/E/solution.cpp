...
void solve() {
  ...
  sort(all(A), greater<ar<ll, 2>>());

  vt<vt<ll>> dp(n+1, vt<ll>(n+1, -1)); // number of positions put on left, right
  ...
  function<ll (int,int)> go = [&](int i, int j) {
    ...
    ll left = go(i+1, j) + A[k][0] * abs(A[k][1] - i);
    ll right = go(i, j+1) + A[k][0] * abs(A[k][1] - (n-1-j));
