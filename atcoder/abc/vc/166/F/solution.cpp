...
void solve() {
  ...
  // dp?
  // dp(i, 3)
  // at the i-th string, we choose to increment j (0 -> a, 1 -> b, 2 -> c)

  const ll INF = 1e18;
  vt<vt<ar<ll, 3>>> dp(n+1, vt<ar<ll, 3>>(3, {-INF, -INF, -INF}));
  ...
  for(int i=0; i<n; i++) {
    ...
    for(auto [x, y]: {make_pair(c, d), make_pair(d, c)}) {
      for(int j=0; j<3; j++) {
        ar<ll, 3> cur = dp[i][j];
        cur[x]++;
        cur[y]--;
        dp[i+1][x] = max(dp[i+1][x], cur);
      }
    }
  }
