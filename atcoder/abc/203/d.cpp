...
bool go(int mid) {
  int medianCnt = k * k / 2 + 1;
  memset(dp, 0, sizeof(dp));
  FOR(n) {
    FOR(j, n) {
      if (grid[i][j] >= mid) {
        // mark if element >= median
        dp[i + 1][j + 1] = 1;
      }
    }
  }
  ...
  for (int i = k; i <= n; i++) {
    for (int j = k; j <= n; j++) {
      int cnt = dp[i][j] - dp[i - k][j] - dp[i][j - k] + dp[i - k][j - k];
      // decrease median until cnt >= medianCnt
      if (cnt < medianCnt) {
        return true;
      }
    }
...
int main() {
  ...
  while (lo <= hi) {
    ...
    debug() << imie(mid) imie(hi) imie(lo);
  }
  cout << hi;
}